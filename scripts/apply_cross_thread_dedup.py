...
__attribute__((visibility(\"hidden\"))) void swipegate_haptic_on_back_invoke_exit() {
    ...
    "on back invoke token lifecycle",
)

p = Path("app/build.gradle.kts")
s = p.read_text()
if 'versionCode = 55' not in s or 'versionName = "0.8.1-dev.13"' not in s:
    raise SystemExit("unexpected app version before dev.14")
s = s.replace("versionCode = 55", "versionCode = 56", 1)
s = s.replace('versionName = "0.8.1-dev.13"', 'versionName = "0.8.1-dev.14"', 1)
old_comment = "// dev.13: HyperRT MADV_DONTNEED page guard + self-healing hook fallback + 750 ms dedup; CI-validated guard build."
new_comment = "// dev.14: cross-thread one-shot Ready/Release dedup token + MADV_DONTNEED page guard."
if old_comment not in s:
    raise SystemExit("dev.13 comment not found")
s = s.replace(old_comment, new_comment, 1)
p.write_text(s)

Path(".github/workflows/apply-cross-thread-release-dedup.yml").unlink(missing_ok=True)
Path("scripts/apply_cross_thread_dedup.py").unlink(missing_ok=True)
